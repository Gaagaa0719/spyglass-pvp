...
export class ActionForm {
     ...
     */
    label(text) {
        this.#formData.label(text);
        return this;
    }

    /**
     * Add the divider to this form.
     */
    divider() {
        this.#formData.divider();
        return this;
    }
...
export class MessageForm {
    ...
    async show(player) {
        ...
        const callback = this.#buttons.get(selection);
...
export class ModalForm {
    ...
    /**
     * @typedef {ModalCallback | ModalDropdownCallback | ModalSliderCallback | ModalTextFieldCallback | ModalToggleCallback} AnyModalCallback
     */

    /** @type {AnyModalCallback[]} */
    #elements = [];
    ...
    slider(label, minimumValue, maximumValue, valueStep, callback, defaultValue) {
        this.#formData.slider(label, minimumValue, maximumValue, { defaultValue, valueStep });
        this.#elements.push(callback);
        return this;
    }
    ...
    async show(player) {
        ...
        for (let i = 0; i < formValues.length; i++) {
            ...
            const callback = this.#elements[i];
            await callback(value, player);
        }
